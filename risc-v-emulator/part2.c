...
void execute_instruction(Instruction instruction,Processor *processor,Byte *memory) {    
    ...
    switch(instruction.opcode.opcode) { /* What do we switch on? */
        /* YOUR CODE HERE */
        case 0x33:
...
void execute_load(Instruction instruction, Processor *processor, Byte *memory) {
    switch(instruction.itype.funct3) { /* What do we switch on? */
        /* YOUR CODE HERE */
         // lb
         case 0x0:
            processor->R[instruction.itype.rd] = bitSigner(load(memory, (processor->R[instruction.itype.rs1]+instruction.itype.imm), LENGTH_BYTE,8), 8);
            processor->PC += 4;
            break;
          ...
          case 0x1:
            ...
            processor->R[instruction.itype.rd] = bitSigner(load(memory, ((sWord)
                ...
                bitSigner(instruction.itype.imm, 12)), LENGTH_HALF_WORD,16), 16);
            break;
        ...
        case 0x2:
            ...
            processor->R[instruction.itype.rd] = load(memory, ((sWord)
                processor->R[instruction.itype.rs1]) + ((sWord)
                bitSigner(instruction.itype.imm, 12)), LENGTH_WORD,32);
            break;
    ...
    switch(instruction.stype.funct3) { /* What do we switch on? */
        ...
        case 0x0:
            ...
            store(memory, ((sWord) processor->R[instruction.stype.rs1]) + ((sWord)
                get_store_offset(instruction)), LENGTH_BYTE,
                  processor->R[instruction.stype.rs2],8);
            break;
        ...
        case 0x1:
            ...
            store(memory, ((sWord) processor->R[instruction.stype.rs1]) + ((sWord)
                ...
                processor->R[instruction.stype.rs2],16);
            break;
        ...
        case 0x2:
            ...
            store(memory, ((sWord) processor->R[instruction.stype.rs1]) + ((sWord)
                get_store_offset(instruction)), LENGTH_WORD,
                  processor->R[instruction.stype.rs2],32);
            break;
